// `import()` is memoised by the module registry, so warming a chunk here makes
// the matching `lazy()` resolve without a round trip.
...
// Held until `load`: requestIdleCallback alone fires within tens of
// milliseconds and would race the first page's own requests.
